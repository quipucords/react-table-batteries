...
- For server-paginated tables: `useTableState` and `useTablePropHelpers`.
  ...
  - The return value (the same `batteries` object returned by `useClientTableBatteries`) has everything you need to render your table. Give it a `console.log` to see what is available or reference [the `TableBatteries` type in the `types.ts` file](https://github.com/quipucords/react-table-batteries/blob/main/packages/module/src/types.ts#L263).
